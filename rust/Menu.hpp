...
namespace menu {
	...
	void BuildMenuMain()
	{
		ImGui::Begin(xorget("External Rust"), NULL, ImVec2(500, 300), 1.00f, NULL);
		if (ImGui::TreeNodeEx(xorget("Esp"), ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			if (config.DrawPlayers) {
				...
				if (ImGui::TreeNodeEx(xorget("World Items"))) {
					...
					if (config.node_esp) {
						if (ImGui::TreeNodeEx(xorget("Nodes Esp"))) {
					...
					if (ImGui::TreeNodeEx(xorget("Find Esp"))) {
		...
		if (ImGui::TreeNodeEx(xorget("Weapon"))) {
			...
			ImGui::Checkbox(xorget("Oneclickniggereoka"), &config.oneeoka);

			ImGui::TreePop();
		}
		if (ImGui::TreeNodeEx(xorget("Misc"))) {
			ImGui::Checkbox(xorget("Spiderman"), &config.spiderman);
			...
			if (config.day_changer)
				ImGui::SliderFloat(xorget("Time"), &config.day_time, 0.f, 24.f);
		...
		if (ImGui::TreeNodeEx(xorget("Unsafe"))) {
			ImGui::Checkbox(xorget("Double Jump"), &config.doublejump);
			ImGui::Checkbox(xorget("Walk On Water"), &config.WalkonWater);
			ImGui::TreePop();
		}
		ImGui::End();
	}
