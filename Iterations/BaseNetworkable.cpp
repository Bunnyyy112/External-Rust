		...
		while (true) {
			...
			try {
				for (auto i = 0; i <= current_object_list_size; i++)
				{
					...
					//player
					if (tag == 6) {
						int pHealth = (int)(kiface::read<float>(BasePlayer + 0x20C));

						uintptr_t StringPlayerName = kiface::read<uintptr_t>(BasePlayer + 0x650);

						uint64_t Inventory = kiface::read<uint64_t>(BasePlayer + 0x608);
						...
						currentplayer.playerflags = kiface::read<int>(currentplayer.baseentity + 0x5F8);
						...
						wep.baseprojectile = kiface::read<uint64_t>(wep.activeweapon + 0x98);
