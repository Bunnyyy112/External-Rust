...
namespace rust {
			...
			while (current_tagged_obj &&
				...
				if (!m_camera) {
					if (tag == 5) {
						...
						VMatrix ViewMatrix = kiface::read<VMatrix>(ent + 0x2e4);
	...
	Vector3 calculate_angle(const Vector3& source, const Vector3& destination)
	{
		...
		return { std::asin(direction.y / direction.Length()) * r2d, -std::atan2(direction.x, -direction.z) * r2d, 1.f };
	}
	...
	void run_aimbot(uintptr_t player) {
		if (config.aimbot) {
			...
			if (GetAsyncKeyState(config.aimbot_key) & 0x8000) {
				...
				angle_to = rust::local->get_viewangles().lerp(angle_to, (2.f - config.aimbot_smooth));
	...
	void entity_esp_thread(ImFont* font, Vector2 res) {
		if (config.DrawPlayers) {
			try {
								...
								if (config.box_esp && !config.cornered_box_esp) {
									if(config.filled_box_esp)
										Renderer::GetInstance()->draw_rectangle(bounds.left,
											...
											ImColor(color.color.Value.x, color.color.Value.y, color.color.Value.z, 6.f));
									else
										Renderer::GetInstance()->draw_rectangle(bounds.left,
											bounds.top,
											bounds.right,
				...
				if (config.Raid_esp) {
					for (Entity Explosion : explosion) {
						DWORD64 visualState = kiface::read<DWORD64>(Explosion.transform + 0x38);
						Vector3 Position = kiface::read<Vector3>(visualState + 0x90);
						float distance = LocalHead.Distance(Position);
						Position = pViewMatrix.WorldToScreen(pViewMatrix, Position, res);
						...
						if (distance > config.max_dist_Raid)
							PassedChecks = false;

						if (!Explosion.networkid)
							PassedChecks = false;

						string Explosion_name;

						if (Explosion.name.find(xorget("assets/prefabs/tools/c4/effects/c4_explosion.prefab")) != std::string::npos)
							Explosion_name = xorget("C4 Explosion");

						if (PassedChecks) {
							float      info_y = Position.y;

							Renderer::GetInstance()->draw_text(Position,
								format(xorget("%s %d"), Explosion_name.c_str(), (int)distance),
								color.color,
								true,
								true,
								font,
				...
				if (config.dropped_esp) {
					for (Entity droppeditem : DroppedItem) {
						...
						if (PassedChecks) {
							if (item_name.size() > 0) {
								Renderer::GetInstance()->draw_text(Position,
									format(xorget("%s"), item_name.c_str(), (int)distance),
									color.color,
									true,
									true,
									font,
				...
				if (config.stash_esp) {
					for (Entity stash : Stash) {
						...
						if (PassedChecks) {
							Renderer::GetInstance()->draw_text(Position,
								format(xorget("%s"), stash_name.c_str(), (int)distance),
								color.color,
								true,
								true,
								font,
				...
				if (config.airdrop) {
					for (Entity airdrop : Airdrop) {
						...
						if (PassedChecks) {
							Renderer::GetInstance()->draw_text(Position,
								format(xorget("%s"), xorget("Airdrop"), (int)distance),
								color.color,
								true,
								true,
								font,
				...
				if (config.hemp) {
					for (Entity Hemp : hemp) {
						...
						if (PassedChecks) {
							Renderer::GetInstance()->draw_text(Position,
								format(xorget("%s"), xorget("Hemp"), (int)distance),
								color.color,
								true,
								true,
								font,
				...
				if (config.patrol_heli) {
					for (Entity PatrolHeli : patrol_heli) {
						...
						if (PassedChecks) {
							Renderer::GetInstance()->draw_text(Position,
								format(xorget("%s"), xorget("Patrol Nigger"), (int)distance),
								color.color,
								true,
								true,
								font,
	...
	bool init() {
		...
		Sleep(8000);
		basenetworkable = kiface::read<uintptr_t>(module_info::gameassembly_base + 0x292AEB8);
		if (!basenetworkable) {
			std::cout << "Bn Outdated" << std::endl;
			return false;
		}

		gameobjectmanager = kiface::read<uintptr_t>(module_info::unityplayer_base + 0x17A6AD8);
		if (!gameobjectmanager) {
			std::cout << "Gom outdated" << std::endl;
			return false;
		}
		return true;
	}
